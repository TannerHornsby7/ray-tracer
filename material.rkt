...
(define (lambertian-material rgb)
  (Material
   no-emission
   (lambda ([ray : Ray] [pt : Float3] [norm : Float3])   
              (Some (Reflect-Info rgb (make-ray pt (fl3+ norm (random-point-in-sphere))))))))
;;Metal Material
...
(define (metal-material rgb fuzz)
  (Material
   no-emission
   (lambda ([ray : Ray] [pt : Float3] [norm : Float3])
             (if (<= (fl3-dot norm (fl3+ (fl3-reflect (Ray-dir ray) norm) (fl3-scale fuzz (random-point-in-sphere)))) 0.0)
              'None
              (Some (Reflect-Info rgb (make-ray pt (fl3+ (fl3-reflect (Ray-dir ray) norm) (fl3-scale fuzz (random-point-in-sphere))))))))))

;;Diffusive/Emissive Material
(: diffuse-emissive-material : RGB -> Material)
(define (diffuse-emissive-material rgb)
  (Material
   (lambda ([ray : Ray] [pt : Float3]) rgb)
   (lambda ([ray : Ray] [pt : Float3] [norm : Float3]) 'None)))
